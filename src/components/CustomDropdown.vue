<template>
  <div class="flex flex-col relative">
    ...
    <button type="button" 
    @click="open = !open" 
    :class="['w-full px-4 py-2 border border-gray-300 rounded-2xl bg-white hover:bg-blue-100 hover:text-blue-700 transition-colors flex justify-between items-center',
    ...
    :disabled="disabled">
      ...
      <svg class="w-5 h-5 text-gray-500 transform transition-transform duration-200" :class="{ 'rotate-180': open }" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
    ...
    <ul v-show="open" class="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-2xl shadow">
      <li v-for="(option, index) in options" :key="option.value" @click="selectOption(option)" class="px-4 py-2 hover:bg-blue-100 hover:text-blue-700 cursor-pointer" :class="{'rounded-t-2xl': index === 0,'rounded-b-2xl': index === options.length - 1}">
        {{ option.label }}
      </li>
    </ul>
